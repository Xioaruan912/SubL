...
func ResolveProviderRules(ctx context.Context, template, name string) ([]NormalizedRule, []string, error) {
	...
	if !ok {
		return nil, nil, errors.New("模板中未找到 rule-provider: " + name)
	}
	var data []byte
	var format string
	var err error
	if strings.EqualFold(def.Type, "file") || (def.URL == "" && def.Path != "") {
		...
		if def.URL == "" {
			return nil, nil, errors.New("rule-provider 缺少 URL")
		}
	...
	rules, warnings, err := ParseRulesWithBehavior(data, format, def.Behavior)
	if err != nil {
		return nil, nil, err
	}
...
func safeLocalProviderPath(raw string) (string, error) {
	...
	if clean == "" || clean == "." || filepath.IsAbs(clean) {
		return "", errors.New("本地 provider path 无效")
	}
	clean = strings.TrimPrefix(clean, "."+string(os.PathSeparator))
	clean = strings.TrimPrefix(filepath.ToSlash(clean), "./")
	if clean == ".." || strings.HasPrefix(clean, "../") {
		return "", errors.New("本地 provider path 越权")
	}
...
func fetchProviderCached(ctx context.Context, rawURL string) ([]byte, string, error) {
	...
	ext := strings.TrimPrefix(strings.ToLower(filepath.Ext(strings.Split(rawURL, "?")[0])), ".")
	if ext == "" {
		ext = "yaml"
	}
	path := filepath.Join(cacheRoot, "providers", key+"."+ext)
	body, _, err := get(ctx, rawURL, 16<<20)
	if err == nil {
		if writeErr := atomicCacheWrite(path, body); writeErr != nil {
			return nil, "", writeErr
		}
		return body, ext, nil
	}
