...
type Target = { name: string; domain: string; group: string }
type Result = Target & { status: 'pending' | 'testing' | 'done' | 'error'; ip: string; countryCode: string; rtt: number; note: string }

const targets: Target[] = [
  { name: 'Cloudflare', domain: 'www.cloudflare.com', group: '网络' },
  { name: 'ChatGPT', domain: 'chatgpt.com', group: 'AI' },
  { name: 'OpenAI', domain: 'openai.com', group: 'AI' },
  { name: 'Claude', domain: 'claude.ai', group: 'AI' },
  { name: 'Anthropic', domain: 'anthropic.com', group: 'AI' },
  { name: 'Discord', domain: 'gateway.discord.gg', group: '社交' },
  { name: 'X', domain: 'x.com', group: '社交' },
  { name: 'Medium', domain: 'medium.com', group: '内容' },
  { name: 'Perplexity', domain: 'www.perplexity.ai', group: 'AI' },
  { name: 'Coinbase', domain: 'coinbase.com', group: '金融' },
  { name: 'Notion', domain: 'notion.so', group: '工具' },
  { name: 'Cloudflare CDN', domain: 'cdnjs.cloudflare.com', group: '开发' },
  { name: 'npm Registry', domain: 'registry.npmjs.org', group: '开发' },
  { name: 'GitLab', domain: 'gitlab.com', group: '开发' },
  { name: 'Crunchyroll', domain: 'crunchyroll.com', group: '媒体' },
]
...
const detect = async (item: Result) => {
  ...
  try {
    const response = await fetch(`https://${item.domain}/cdn-cgi/trace?_=${Date.now()}`, { cache: 'no-store', signal: controller.signal })
    ...
    if (!trace.ip) throw new Error('目标未返回出口 IP')
    item.ip = trace.ip; item.countryCode = (trace.loc || '').toUpperCase(); item.rtt = Math.max(1, Math.round(performance.now() - started)); item.status = 'done'
  } catch (error: any) {
...
const runSelected = async () => {
  ...
  try {
    ...
    results.value = (data?.results || []).map((item: any) => ({ name:item.name, domain:item.domain, group:names[item.group] || item.group, status:item.status === 'available' ? 'done' : 'error', ip:item.ip || '', countryCode:item.countryCode || '', rtt:item.rtt ?? -1, note:item.note || '未获取出口 IP' }))
  ...
  <div class="egress-page">
    ...
    <section class="result-card">
      ...
      <el-table :data="results" row-key="domain" class="result-table">
        <el-table-column label="目标" min-width="170"><template #default="{ row }"><div class="target-cell"><span>{{ row.name }}</span><small>{{ row.domain }}</small></div></template></el-table-column>
        ...
        <el-table-column label="状态" min-width="150"><template #default="{ row }"><el-tag :type="statusType(row)" size="small" :effect="row.status === 'testing' ? 'plain' : 'light'">{{ statusText(row) }}</el-tag></template></el-table-column>
      </el-table>
