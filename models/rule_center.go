...
// RuleCacheSnapshot keeps the previous verified payload before an explicit
// rule update so administrators can roll back without depending on the remote.
type RuleCacheSnapshot struct {
	ID            uint      `gorm:"primaryKey" json:"id"`
	ExternalID    string    `gorm:"size:512;index;not null" json:"externalId"`
	Checksum      string    `gorm:"size:128" json:"checksum"`
	RuleCount     int       `json:"ruleCount"`
	MetadataJSON  string    `gorm:"type:text" json:"metadataJson"`
	CacheRevision string    `gorm:"size:128" json:"cacheRevision"`
